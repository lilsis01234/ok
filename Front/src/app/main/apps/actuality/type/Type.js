...
function ModernComingSoonPage() {
  ...
  const [idTypeEdit, setidTypeEdit] = useState('');
...
const handleSubmit = async (e) => { e.preventDefault();
  ...
  if (nomInput) {
    ...
    const serveurApi = 'http://localhost:4001/api/actualite/type/new';
    axios.post(serveurApi, dataForm)
        .then(res => {
          if (res.data) {
              setNouveauType(res.data.nom);
...
const deleteType = (id) => {
  ...
  axios.delete(`http://localhost:4001/api/actualite/type/${id}/delete`)

  .then(() => {
    const updatedType = listeType.filter(type => type.id !== id);
...
const handleEdit = (e) => {
  ...
  if(nom) {
    ...
    axios.put(`http://localhost:4001/api/actualite/type/${id}/edit`, dataForm)
    ...
    <div className="flex flex-col flex-auto items-center sm:justify-center min-w-0 md:p-32">
      <Paper className="flex w-full sm:w-auto min-h-full sm:min-h-auto md:w-full md:max-w-6xl rounded-0 sm:rounded-2xl sm:shadow overflow-hidden">
        <div className="w-full sm:w-auto py-32 px-16 sm:p-48 md:p-64 ltr:border-r-1 rtl:border-l-1">
          <div className="w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
            ...
            Une formulaire de gestion qui permet aux utilisateurs d'ajouter des types d'actualités.
            </Typography>
            ...
            <Table aria-label="simple table">
                ...
                {listeType.map((n) => (
                    ...
                    <StyledTableCell  align="center">
                        <IconButton aria-label="edit" color="secondary"  onClick={ () => editType(n.id, n.nom)} >
                            <EditIcon />
                        </IconButton>
      ...
      <Modal open={isDeleteModalOpen} onClose={closeDeleteModal}>
        <Box
          sx={style}
        >
          <p>Êtes-vous sûr de vouloir supprimer cet type d'actualité ?</p>
