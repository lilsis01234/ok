...
function DepartementTableHeader(props) {
    const { selectedDepartementId } = props;
    const numSelected = selectedDepartementId.length;

    const [selectedDepartementMenu, setselectedDepartementMenu] = useState(null);
    ...
    const createSortHandler = (property) => (event) => {
      ...
      function openSelectedDepartementMenu(event) {
        setselectedDepartementMenu(event.currentTarget);
      }

      function closeSelectedDepartementMenu(){
        setselectedDepartementMenu(null);
      }
                ...
                {numSelected > 0 && (
                        <Box className="flex items-center justify-center absolute w-64 top-0 ltr:left-0 rtl:right-0 mx-56 h-64 z-10 border-b-1"
                            ...
                            <Menu
                                ...
                                anchorEl={selectedDepartementMenu}
                                open={Boolean(selectedDepartementMenu)}
                                onClose={closeSelectedDepartementMenu}
                            >
                                <MenuList>
                                    <MenuItem
                                        onClick={() => {
...
export default DepartementTableHeader
